...
import com.tianji.aigc.client.trade.TradeClient;
...
public class OrderTools {
    ...
    public PrePlaceOrder prePlaceOrder(@ToolParam(description = Constant.ToolParams.COURSE_IDS) List<Number> courseIds,
        ...
        var token = MapUtil.getStr(toolContext.getContext(), Constant.AUTHORIZATION);
        if (StrUtil.isNotBlank(token)) {
            TokenContext.setToken(token);
        }
        try {
            // 调用预下单接口
            var orderConfirmVO = this.tradeClient.prePlaceOrder(
                    CollStreamUtil.toList(courseIds, Number::longValue));
